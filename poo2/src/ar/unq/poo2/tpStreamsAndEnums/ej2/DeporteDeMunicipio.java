...
public class DeporteDeMunicipio {
	...
	public ActividadSemanal actividadDeMenorCostoDe(Deporte futbol) {
		return this.getActividades().stream().min(Comparator.comparing((ActividadSemanal act) -> act.precioDeActividad())).get();
	}
	
	public void imprimirTodasLasActividades() {
		this.getActividades().stream().forEach(act -> System.out.println(act.toString()));
	}
}
